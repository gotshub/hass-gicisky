"""Support for Gicisky event entities."""
...
class GiciskyEventEntity(EventEntity):
    """Representation of a Gicisky event entity."""
    ...
    def __init__(
        ...
        address: str,
        event_class: str,
        event: GiciskyBleEvent | None,
    ) -> None:
        """Initialise a Gicisky event entity."""
    ...
    @callback
    def _async_handle_event(self, event: GiciskyBleEvent) -> None:
        self._trigger_event(event[EVENT_TYPE], event[EVENT_PROPERTIES])
...
async def async_setup_entry(
    ...
    """Set up Gicisky event."""
    coordinator = entry.runtime_data
    address = coordinator.address
    ent_reg = er.async_get(hass)
    async_add_entities(
        # Matches logic in PassiveBluetoothProcessorEntity
        GiciskyEventEntity(address_event_class[0], address_event_class[2], None)
    ...
    @callback
    def _async_discovered_event_class(event_class: str, event: GiciskyBleEvent) -> None:
        """Handle a newly discovered event class with or without a postfix."""
        async_add_entities([GiciskyEventEntity(address, event_class, event)])
